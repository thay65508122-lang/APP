...
function updateClock() {
  ...
  if (!el) return;
  const now = new Date();
  el.textContent = now.toLocaleDateString('es-MX', { weekday: 'long', year: 'numeric', month: 'long', day: 'numeric', hour: '2-digit', minute: '2-digit' });
}
...
async function loadPage(page) {
  ...
  container.innerHTML = '<div style="text-align:center;padding:60px;color:var(--text-muted)">⏳ Cargando...</div>';

  try {
    switch (page) {
      case 'dashboard': await renderDashboard(container); break;
      case 'productos': await renderProductos(container); break;
      case 'sucursales': await renderSucursales(container); break;
      case 'inventario': await renderInventario(container); break;
      case 'usuarios': await renderUsuarios(container); break;
      case 'horarios': await renderHorarios(container); break;
      case 'clientes': await renderClientes(container); break;
      case 'proveedores': await renderProveedores(container); break;
      default: container.innerHTML = '<div class="empty-state"><div class="icon">📄</div><h4>Página no encontrada</h4></div>';
    }
  } catch (e) {
    container.innerHTML = `<div class="empty-state"><div class="icon">⚠️</div><h4>Error al cargar</h4><p>${e.message}</p></div>`;
  }
  updateAssistant();
}
  ...
  try {
    ...
    body.innerHTML = `
      <div class="assistant-message">
        <p><strong>👋 Buenas, ${(session?.nombre || 'Usuario').split(' ')[0]}</strong></p>
        <p>Resumen general del sistema</p>
        <span class="msg-time">${timeStr}</span>
      </div>
      <div class="assistant-stats">
        <div class="assistant-stat"><h6>${stats.totalProductos}</h6><p>Productos</p></div>
        <div class="assistant-stat"><h6>${stats.totalItems}</h6><p>Piezas en stock</p></div>
        <div class="assistant-stat"><h6>${UI.formatCurrency(stats.totalValorInventario)}</h6><p>Valor inventario</p></div>
        <div class="assistant-stat"><h6>${stats.totalEmpleados}</h6><p>Empleados</p></div>
      </div>
      ...
      <div class="assistant-message" style="border-left-color:var(--success)">
        <p>✅ <strong>${stats.totalSucursales}</strong> sucursales activas</p>
        <p>💰 Ganancia potencial: ${UI.formatCurrency(stats.gananciaPotencial)}</p>
        <span class="msg-time">Datos en tiempo real</span>
      </div>`;
  } catch {
    body.innerHTML = '<div class="assistant-message"><p>Conectando al servidor...</p></div>';
  }
}
...
async function renderDashboard(container) {
  ...
  container.innerHTML = `
    ...
    <div class="grid-2">
      <div class="card">
        <div class="card-header"><h4>📋 Últimos Movimientos</h4></div>
        <div class="table-container">
          ${stats.movimientos.length === 0
            ? '<div class="empty-state"><p>Sin movimientos registrados</p></div>'
            : `<table><thead><tr><th>Fecha</th><th>Tipo</th><th>Producto</th><th>Sucursal</th><th>Cantidad</th></tr></thead>
              <tbody>${stats.movimientos.map(m => `<tr>
      ...
      <div class="card">
        <div class="card-header"><h4>⚠️ Stock Bajo</h4></div>
        ${stats.bajoStock.length === 0
          ? '<div class="empty-state"><p>✅ Todos los productos tienen stock suficiente</p></div>'
          : `<div class="table-container"><table><thead><tr><th>Producto</th><th>Sucursal</th><th>Stock</th></tr></thead>
            <tbody>${stats.bajoStock.map(b => `<tr>
              <td>${b.productoNombre}</td><td>${b.sucursalNombre}</td>
              <td><span class="tag tag-warning">${b.cantidad}</span></td>
            </tr>`).join('')}</tbody></table></div>`}
      </div>
    </div>
    <div class="card">
      <div class="card-header"><h4>💎 Productos por Metal</h4></div>
      <div style="display:flex;gap:16px;flex-wrap:wrap">
        ${stats.productosPorMetal.map(({ metal, count }) =>
...
async function renderProductos(container) {
  const [productos, inventario] = await Promise.all([API.getProductos(), API.getInventario()]);
  ...
  container.innerHTML = `
    ...
    <div class="card">
      <div class="table-container">
        <table>
          <thead><tr><th>Producto</th><th>Tipo</th><th>Metal</th><th>Pureza</th><th>Peso</th><th>Gema</th><th>Año</th><th>Precio</th><th>Stock Total</th><th>Acciones</th></tr></thead>
          <tbody id="productosTableBody">
            ${productos.map(p => {
              const stockTotal = inventario.filter(i => i.productoId === p.id).reduce((s, i) => s + i.cantidad, 0);
              return `<tr>
  ...
  window.showFormProducto = async (id = null) => {
    let p = null;
    ...
    const { modal, body, footer, close } = UI.modalDynamic(title, (b) => {
      ...
      f.querySelector('#btnSaveProducto').addEventListener('click', async () => {
        const data = {
        ...
        try {
          if (id) { await API.updateProducto(id, data); UI.toast('Producto actualizado', 'success'); }
          else { await API.addProducto(data); UI.toast('Producto creado', 'success'); }
          close();
          await renderProductos(container);
  ...
  window.deleteProducto = async (id) => {
    ...
    if (!p) return;
    UI.confirmDialog(`¿Eliminar "${p.nombre}"?`, async () => {
      try { await API.deleteProducto(id); UI.toast('Producto eliminado', 'success'); await renderProductos(container); }
      catch (e) { UI.toast(e.message, 'error'); }
    });
  };
}
...
async function renderSucursales(container) {
  ...
  container.innerHTML = `
    ...
    <div class="stats-grid" style="grid-template-columns:repeat(auto-fit,minmax(280px,1fr))">
      ${sucursales.map(s => {
        const emp = usuarios.filter(u => u.sucursalId === s.id);
        return `<div class="card" style="margin:0">
          ...
          <div class="stats-grid" style="grid-template-columns:1fr 1fr;gap:8px;margin:0">
            <div class="assistant-stat"><h6>${s.totalItems}</h6><p>Piezas</p></div>
            <div class="assistant-stat"><h6>${UI.formatCurrency(s.totalValor)}</h6><p>Valor</p></div>
            <div class="assistant-stat"><h6>${emp.length}</h6><p>Empleados</p></div>
            <div class="assistant-stat"><h6 class="${s.totalItems < 20 ? 'trend down' : ''}">${s.bajoStock}</h6><p>Stock bajo</p></div>
          </div>
        </div>`;
      }).join('')}
    </div>`;

  window.showFormSucursal = (id = null) => {
    const s = id ? sucursales.find(s => s.id === id) : null;
    ...
    window.saveSucursal = async (id) => {
      ...
      if (!data.nombre) { UI.toast('El nombre es obligatorio', 'error'); return; }
      try {
        if (id) { await API.updateSucursal(id, data); UI.toast('Sucursal actualizada', 'success'); }
        else { await API.addSucursal(data); UI.toast('Sucursal creada', 'success'); }
        m.closest('.modal-overlay').remove();
        await renderSucursales(container);
      } catch (e) { UI.toast(e.message, 'error'); }
    };
  };

  window.deleteSucursal = (id) => {
    const s = sucursales.find(s => s.id === id);
    if (!s) return;
    UI.confirmDialog(`¿Desactivar "${s.nombre}"?`, async () => {
      try { await API.deleteSucursal(id); UI.toast('Sucursal desactivada', 'success'); await renderSucursales(container); }
      catch (e) { UI.toast(e.message, 'error'); }
    });
  };
}
...
async function renderInventario(container) {
  const [inventario, sucursales, productos] = await Promise.all([
    API.getInventario(), API.getSucursales(), API.getProductos()
  ]);
  ...
  container.innerHTML = `
    ...
    <div class="card">
      <div class="table-container">
        <table>
          <thead><tr><th>Sucursal</th><th>Producto</th><th>Tipo</th><th>Metal</th><th>Cantidad</th><th>Valor Total</th><th>Estado</th><th>Acciones</th></tr></thead>
          <tbody id="inventarioTableBody">
            ${inventario.map(item => {
              const valor = (item.precio || 0) * item.cantidad;
              ...
              return `<tr>
                <td><strong>${item.sucursalNombre}</strong></td>
                <td>${item.productoNombre}</td>
                <td>${UI.tipoProducto(item.tipo)}</td>
                <td>${UI.metalTag(item.metal)}</td>
                <td>${item.cantidad}</td>
  ...
  window.filterInventario = () => {
    ...
    document.querySelectorAll('#inventarioTableBody tr').forEach(row => {
  ...
  window.ajustarStock = (id) => {
    ...
    const m = UI.modal('Ajustar Stock',
      `<div class="form-group"><label>Sucursal</label><input type="text" value="${item.sucursalNombre}" disabled style="opacity:0.6"></div>
       <div class="form-group"><label>Producto</label><input type="text" value="${item.productoNombre}" disabled style="opacity:0.6"></div>
    ...
    window.saveStock = async (id) => {
      ...
      try { await API.updateInventario(id, { cantidad: cant }); UI.toast('Stock actualizado', 'success'); m.closest('.modal-overlay').remove(); await renderInventario(container); }
      catch (e) { UI.toast(e.message, 'error'); }
    };
  };

  window.showReabastecer = () => {
    const m = UI.modal('Reabastecer Sucursal',
      `<div class="form-group"><label>Sucursal</label>
        <select id="rbSucursal">${sucursales.map(s => `<option value="${s.id}">${s.nombre}</option>`).join('')}</select>
      </div>
      <div class="form-group"><label>Producto</label>
        <select id="rbProducto">${productos.map(p => `<option value="${p.id}">${p.nombre} - ${UI.formatCurrency(p.precio)}</option>`).join('')}</select>
      </div>
      <div class="form-group"><label>Cantidad a agregar</label><input type="number" id="rbCantidad" value="5" min="1" step="1"></div>`,
      `<button class="btn btn-outline" onclick="this.closest('.modal-overlay').remove()">Cancelar</button>
       <button class="btn btn-success" onclick="doReabastecer()">Reabastecer</button>`
    );
    window.doReabastecer = async () => {
      const data = {
        sucursalId: document.getElementById('rbSucursal').value,
        productoId: document.getElementById('rbProducto').value,
        cantidad: parseInt(document.getElementById('rbCantidad').value)
      };
      if (!data.cantidad || data.cantidad < 1) { UI.toast('Cantidad inválida', 'error'); return; }
      try { await API.reabastecer(data); UI.toast('Inventario reabastecido', 'success'); m.closest('.modal-overlay').remove(); await renderInventario(container); }
      catch (e) { UI.toast(e.message, 'error'); }
    };
  };
}
...
async function renderUsuarios(container) {
  const [usuarios, sucursales] = await Promise.all([API.getUsuarios(), API.getSucursales()]);
  const roles = [
        ...
        <table>
          ...
          <tbody>
            ${usuarios.map(u => `<tr>
              ...
              <td>${UI.sucursalNombre(u.sucursalId, sucursales)}</td>
  ...
  window.showFormUsuario = (id = null) => {
    ...
    const m = UI.modal(title,
      `<div class="grid-2">
        ...
        ${UI.formField('Contraseña' + (u ? ' (dejar vacío para mantener)' : ''), 'password', 'fuPassword', '')}
    ...
    window.saveUsuario = async (id) => {
      ...
      try {
        if (id) { await API.updateUsuario(id, data); UI.toast('Usuario actualizado', 'success'); }
        else {
          if (!pass) { UI.toast('La contraseña es obligatoria', 'error'); return; }
          await API.addUsuario(data);
  ...
  window.deleteUsuario = (id) => {
    ...
    if (!u) return;
    UI.confirmDialog(`¿Desactivar a "${u.nombre}"?`, async () => {
      try { await API.deleteUsuario(id); UI.toast('Usuario desactivado', 'success'); await renderUsuarios(container); }
      catch (e) { UI.toast(e.message, 'error'); }
    });
  };
}
...
async function renderHorarios(container) {
  const [horarios, operariosRaw, sucursales] = await Promise.all([
    API.getHorarios(), API.getUsuarios(), API.getSucursales()
  ]);
  const operarios = operariosRaw.filter(u => u.rol === 'operario' || u.rol === 'gerente');
  const dias = ['Domingo', 'Lunes', 'Martes', 'Miércoles', 'Jueves', 'Viernes', 'Sábado'];

  window._reloadHorarios = () => { const c = document.getElementById('pageContent'); if (c) renderHorarios(c); };
      ...
      <div class="table-container">
        <table>
          ...
          <tbody id="horariosTableBody">
            ${horarios.map(h => {
              const filtroVal = document.getElementById('filterOperario')?.value;
              if (filtroVal && h.operarioId !== filtroVal) return '';
              return `<tr>
                <td><strong>${h.operarioNombre}</strong></td>
  ...
  window.showFormHorario = (id = null) => {
    const h = id ? horarios.find(h => h.id === id) : null;
    const title = h ? 'Editar Horario' : 'Asignar Horario';
    const m = UI.modal(title,
      `<div class="form-group"><label>Operario</label>
        <select id="fhOperario">${operarios.map(o => `<option value="${o.id}" ${h?.operarioId === o.id ? 'selected' : ''}>${o.nombre}</option>`).join('')}</select>
      </div>
      <div class="form-group"><label>Sucursal</label>
        <select id="fhSucursal">${sucursales.map(s => `<option value="${s.id}" ${h?.sucursalId === s.id ? 'selected' : ''}>${s.nombre}</option>`).join('')}</select>
      </div>
      <div class="grid-3">
        <div class="form-group"><label>Día</label>
          <select id="fhDia">${dias.map((d, i) => `<option value="${i}" ${h?.dia === i ? 'selected' : ''}>${d}</option>`).join('')}</select>
        </div>
        <div class="form-group"><label>Hora Inicio</label><input type="time" id="fhInicio" value="${h?.horaInicio || '09:00'}"></div>
        <div class="form-group"><label>Hora Fin</label><input type="time" id="fhFin" value="${h?.horaFin || '18:00'}"></div>
      </div>`,
      `<button class="btn btn-outline" onclick="this.closest('.modal-overlay').remove()">Cancelar</button>
       <button class="btn btn-primary" onclick="saveHorario('${id || ''}')">${h ? 'Actualizar' : 'Asignar'}</button>`
    );
    window.saveHorario = async (id) => {
      const data = {
      ...
      try {
        if (id) { await API.updateHorario(id, data); UI.toast('Horario actualizado', 'success'); }
        else { await API.addHorario(data); UI.toast('Horario asignado', 'success'); }
        m.closest('.modal-overlay').remove();
        await renderHorarios(container);
      } catch (e) { UI.toast(e.message, 'error'); }
    };
  };

  window.deleteHorario = (id) => {
    UI.confirmDialog('¿Eliminar este horario?', async () => {
      try { await API.deleteHorario(id); UI.toast('Horario eliminado', 'success'); await renderHorarios(container); }
      catch (e) { UI.toast(e.message, 'error'); }
    });
  };
}
...
async function renderClientes(container) {
  ...
  window.showFormCliente = (id = null) => {
    const c = id ? clientes.find(c => c.id === id) : null;
    ...
    window.saveCliente = async (id) => {
      ...
      if (!data.nombre) { UI.toast('El nombre es obligatorio', 'error'); return; }
      try {
        if (id) { await API.updateCliente(id, data); UI.toast('Cliente actualizado', 'success'); }
        else { await API.addCliente(data); UI.toast('Cliente registrado', 'success'); }
        m.closest('.modal-overlay').remove();
  ...
  window.deleteCliente = (id) => {
    UI.confirmDialog('¿Eliminar este cliente?', async () => {
      try { await API.deleteCliente(id); UI.toast('Cliente eliminado', 'success'); await renderClientes(container); }
      catch (e) { UI.toast(e.message, 'error'); }
    });
  };
}
...
async function renderProveedores(container) {
  ...
  window.showFormProveedor = (id = null) => {
    const p = id ? proveedores.find(p => p.id === id) : null;
    ...
    window.saveProveedor = async (id) => {
      ...
      try {
        if (id) { await API.updateProveedor(id, data); UI.toast('Proveedor actualizado', 'success'); }
        else { await API.addProveedor(data); UI.toast('Proveedor registrado', 'success'); }
        m.closest('.modal-overlay').remove();
        await renderProveedores(container);
      } catch (e) { UI.toast(e.message, 'error'); }
    };
  };

  window.deleteProveedor = (id) => {
    UI.confirmDialog('¿Eliminar este proveedor?', async () => {
      try { await API.deleteProveedor(id); UI.toast('Proveedor eliminado', 'success'); await renderProveedores(container); }
      catch (e) { UI.toast(e.message, 'error'); }
    });
  };
}
